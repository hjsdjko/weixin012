	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussshicaixinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussshicaixinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussshicaixinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discussshicaixinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussshicaixinxiCleanType',true);
