...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshicaixinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussshicaixinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussshicaixinxi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshicaixinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussshicaixinxi');
			...
			async init(){
				...
				let res = await this.$api.info('discussshicaixinxi', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussshicaixinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussshicaixinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
