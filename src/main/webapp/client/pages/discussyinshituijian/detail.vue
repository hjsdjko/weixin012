...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyinshituijianCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyinshituijianCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussyinshituijian', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussyinshituijian');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussyinshituijian');
			...
			async init(){
				...
				let res = await this.$api.info('discussyinshituijian', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussyinshituijian', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussyinshituijian/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
