	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('shicaifenlei', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','shicaifenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','shicaifenlei');
			...
			async init(){
				...
				let res = await this.$api.info('shicaifenlei', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussshicaifenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
