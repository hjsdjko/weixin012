...
<view class="content">
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"#ffffff","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0px 0px 2rpx 0px","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="" @tap="shiwutupianTap">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333333","textAlign":"right"}' class="title">食物图片</view>
				<image :style='{"width":"80rpx","borderRadius":"100%","objectFit":"cover","display":"block","height":"80rpx"}' class="avator" v-if="ruleForm.shiwutupian" :src="baseUrl+ruleForm.shiwutupian.split(',')[0]" mode="aspectFill"></image>
			...
			<view :style='{"padding":"12rpx 0 12rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","borderWidth":"0 0 0px 0","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"100%","lineHeight":"80rpx","fontSize":"28rpx","color":"#333333","fontWeight":"500"}' class="title">食物详情</view>
                <xia-editor ref="editor" :style='{"minHeight":"300rpx","border":"2rpx solid #eeeeee","padding":"20rpx","color":"#666666","background":"none","width":"100%","height":"auto"}' v-model="ruleForm.shiwuxiangqing" placeholder="食物详情" @editorChange="shiwuxiangqingChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yinshituijian`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					...
					if(o=='shiwuxiangqing'){
					this.ruleForm.shiwuxiangqing = obj[o];
					this.ro.shiwuxiangqing = true;
					continue;
					}
					if(o=='storeupnum'){
		...
		methods: {
            shiwuxiangqingChange(e) {
                this.ruleForm.shiwuxiangqing = e
            },
			styleChange() {
				this.$nextTick(()=>{
			...
			shiwutupianTap() {
				...
				this.$api.upload(function(res) {
					_this.ruleForm.shiwutupian = 'file/' + res.file;
					_this.$forceUpdate();
					_this.$nextTick(()=>{
						_this.styleChange()
					})
...
//跨表计算判断
				var obj;
				if((!this.ruleForm.shiwutupian)){
					this.$utils.msg(`食物图片不能为空`);
					return
				}
				if((!this.ruleForm.shiwumingcheng)){
					this.$utils.msg(`食物名称不能为空`);
					return
				}
				...
				if((!this.ruleForm.kouwei)){
					this.$utils.msg(`口味不能为空`);
					return
				}
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`yinshituijian`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`yinshituijian`, this.ruleForm);
						}else{
							await this.$api.add(`yinshituijian`, this.ruleForm);
						}
